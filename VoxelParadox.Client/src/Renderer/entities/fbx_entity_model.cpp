...
void appendSceneMeshVertices(const aiMesh& mesh, const glm::mat4& nodeTransform,
    ...
    for (unsigned int faceIndex = 0; faceIndex < mesh.mNumFaces; ++faceIndex) {
        ...
        for (unsigned int localIndex = 0; localIndex < 3; ++localIndex) {
            ...
            const glm::vec3 position = toGlm(mesh.mVertices[vertexIndex]);
            const glm::vec3 modelPosition =
                glm::vec3(nodeTransform * glm::vec4(position, 1.0f));
            const glm::vec3 normal = mesh.HasNormals()
                                         ? glm::normalize(toGlm(mesh.mNormals[vertexIndex]))
            ...
            outPart.vertices.push_back(EntityModelVertex{position, normal, uv});
            outPart.boundsMin = glm::min(outPart.boundsMin, position);
            outPart.boundsMax = glm::max(outPart.boundsMax, position);
            modelBoundsMin = glm::min(modelBoundsMin, modelPosition);
            modelBoundsMax = glm::max(modelBoundsMax, modelPosition);
        }
    }
}
...
void appendNodeMeshesRecursive(const aiScene& scene, const aiNode* node,
    ...
    for (unsigned int localMeshIndex = 0; localMeshIndex < node->mNumMeshes; ++localMeshIndex) {
        ...
        EntityModelPartAsset part{};
        part.name = pickPartName(node, *mesh, sceneMeshIndex);
        part.nodeTransform = nodeTransform;
...
bool loadFbxEntityModel(const std::filesystem::path& modelAssetPath,
    ...
    if (!scene || !scene->HasMeshes()) {
        outError = "Failed to parse FBX model: " + modelAbsolutePath.string() +
    ...
    if (outModel.texturePath.empty()) {
        outError = "Failed to resolve FBX texture next to model: " +
                   modelAbsolutePath.string();
        return false;
    }
    ...
    if (outModel.parts.empty()) {
        outError = "FBX import produced no triangles: " + modelAbsolutePath.string();
        return false;
    }

    if (enemyType == EnemyType::Guy) {
        // The gameplay spawn yaw points away from the player camera.
        // Keep the humanoid front visible after spawning on a targeted block.
        outModel.yawOffsetDegrees = 180.0f;
    }
